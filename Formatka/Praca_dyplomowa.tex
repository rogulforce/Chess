...
W późniejszej części pracy pojawią się wykresy i analizy, w których dziedziną będzie numer wykonywanego ruchu. W związku z małą liczbą danych dla późniejszych ruchów, analizowane będą jedynie te poniżej 58, jako, że wynik 95\% gier pojawia się maksymalnie w 57 ruchach. Rozkład długości gier z bazy danych przedstawiony jest na rysunku \ref{rys:dlugosc_gier}.
...
Pierwszym elementem analizy, który pozwoli na określenie wstępnej różnicy między ruchami oznaczonymi jako ,,duży błąd'', a zbiorem wszystkich ruchów jest sprawdzenie zależności między ruchem, a średnim czasem na jego wykonanie dla obydwu zbiorów. Intuicja każe wskazywać, że błąd popełniamy poświęcając mniej czasu na ruch. Jak widać na rysunku \ref{rys:sr_czas_na_ruch}, dla każdego analizowanego ruchu, ruch oznaczony jako błędny zajął statystycznie więcej czasu - zarówno dla formatu 300+0 jak i 600+0, co przeczy intuicji.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{sr_czas_na_ruch.png}
	\caption{średni czas na wykonanie ruchu dla analizowanych formatów czasowych. Osobno wszystkie ruchy i ruchy oznaczone przez silnik jako ,,duży błąd''.}
